      ...
      <Card className="da2-req-block bg-input-background border-border p-0 overflow-hidden">
        ...
        {sectionIsExpanded && (
          <div className="da2-rb-courses">
            {(() => {
              // Group rows: collect consecutive OR choices from the same block together,
              ...
              sectionCourseRows.forEach((row) => {
                if (row.reqType.startsWith("OR_BLOCK::")) {
                  ...
                  let choiceGroup = currentOrBlock.choices.find((c) => c.choiceIndex === choiceIndex);
                  if (!choiceGroup) {
                    choiceGroup = { choiceIndex, rows: [] };
                    currentOrBlock.choices.push(choiceGroup);
                  }
                  choiceGroup.rows.push(row);
                } else {
                  currentOrBlock = null;
                  groups.push({ kind: "required", row });
                }
              });

              const renderCourseRow = (row: (typeof sectionCourseRows)[0], showReqType?: string) => {
              ...
              return groups.map((group, groupIndex) => {
                if (group.kind === "required") {
                  return renderCourseRow(group.row, "Required");
                }

                // OR block: render all choices with "— or —" dividers between them
                return (
                  <div
                    key={`or-block-${group.blockKey}-${groupIndex}`}
                    className="da2-or-block"
                    role="group"
                    ...
                    <div className="da2-or-choices">
                      ...
                      {group.choices.map((choice, choiceIdx) => (
                        <div key={`choice-${choiceIdx}`} className="da2-or-choice-group">
                          {choiceIdx > 0 && (
                            <div className="da2-or-divider" aria-hidden="true">
                              <span className="da2-or-divider-line" />
                              <span className="da2-or-divider-label">or</span>
                              <span className="da2-or-divider-line" />
                            </div>
                          )}
                          {choice.rows.map((row) => renderCourseRow(row))}
                        </div>
                      ))}
                    </div>
                  </div>
